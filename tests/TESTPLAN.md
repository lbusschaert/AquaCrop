...
| B17 | Perennial: onset generated, TMeanPeriod (code 12) | `AirTCriterion_TMeanPeriod` | T2 | [n] |
| B18 | Perennial: onset generated, GDDPeriod (code 13) | `AirTCriterion_GDDPeriod` | T2 | [n] |
| B19 | Perennial: end generated by air-T criterion | `PerennialPeriod_GenerateEnd` | T2 | [n] |
| B20 | Perennial: onset occurrence 1 vs 2 vs 3 | `OnsetOccurrence` | T2 | [n] |
| B21 | Perennial: onset occurrence > 3 (clamped) | clamp to 3 | T3 | [n] |
| B22 | Perennial: extra years beyond the record | `OnsetExtraYears` | T2 | [n] |
...
| P17 | Cycle longer than 365 days | multi-year annual cycle | T3 | [n] |
...
| Q03 | `theta_x <= SAT` with theta above `theta_x` | first sub-branch | T1 | [n] |
| Q04 | `theta_x <= SAT` with theta between FC and `theta_x` | second sub-branch | T1 | [n] |
...
| Q08 | Excess reaching compartment 1 | `pre_nr == 1` loop exit | T1 | [n] |
| Q09 | Excess exactly zero | `abs(excess) < epsilon` exit | T3 | [n] |
...
| Q14 | `TauFromKsat` at its breakpoints | the tau lookup | T2 | [n] |
...
| R02 | Decadal rain record, shower-based runoff | the non-daily branch | T1 | [n] |
| R03 | Runoff ≥ shower depth | `Runoff >= Shower` clamp | T2 | [n] |
| R04 | Runoff computed greater than rain | `Runoff > Rain` clamp | T2 | [n] |
| R05 | Initial abstraction exceeding rain | `term <= epsilon` → zero runoff | T2 | [n] |
| R06 | Profile drier than WP in the runoff depth | the `theta < WP` weighting branch | T2 | [ ] |
| R07 | Weighted wetness sum < 0 | `SUM < 0` clamp | T3 | [n] |
...
| R19 | `delta_theta_nul < delta_theta_SAT` | the first infiltration sub-branch | T1 | [n] |
| R20 | `theta_nul <= FCadj` | clamping to adjusted FC | T1 | [n] |
| R21 | `theta_nul > SAT` | clamping to saturation | T1 | [n] |
| R22 | `fluxout + drain_max` exceeded | flux limiting at a compartment | T1 | [n] |
| R23 | Infiltration excess pushed back up | `excess > 0`, `pre_comp` walk | T1 | [n] |
| R24 | Excess reaching compartment 1 → runoff | `Runoff > RunoffIni` | T1 | [n] |
| R25 | Excess computed negative | the `excess < 0` guard | T3 | [n] |
| R26 | Water still to store after the whole profile | `amount_still_to_store > 0` at the end | T1 | [ ] |
| R27 | Exactly enough storage capacity | `amount_still_to_store <= epsilon` exit | T2 | [n] |
...
| R34 | Rain on the last day of the simulation | end-of-record handling | T2 | [n] |
| R35 | Effective rainfall, percentage method, with `SubDrain` | `CalculateEffectiveRainfall` sub-drain path | T1 | [n] |
| R36 | Effective rain with surface storage present | `SurfaceStorage > 0` branch | T2 | [ ] |
| R37 | Effective rain with `Zr <= 0` (no roots yet) | the zero-root-depth guard | T2 | [n] |
| R38 | Effective rain with `RestTheta <= 0` | the saturated-root-zone guard | T3 | [n] |
...
| R41 | `EffecRain` computed negative | the `EffecRain < 0` clamp | T3 | [n] |
| R42 | `EffecRain` exceeding rain − runoff | the upper clamp | T3 | [n] |
...
| S09 | `EvapZmax` == `EvapZmin` | the guard in stage II | T3 | [n] |
| S10 | Evaporation layer spanning two soil layers | `WCEvapLayer` across a boundary | T1 | [ ] |
| S11 | Evaporation layer thinner than compartment 1 | sub-compartment extraction | T2 | [n] |
| S12 | Top compartment reaching WP/2 | the `theta <= WP/200` floor | T2 | [ ] |
| S13 | Salt cell walk ending below the previous cell | `SCellEnd < SCellIniEvap(compi-1)` | T2 | [n] |
| S14 | Salt cell walk reaching `SCP1` | `SCellEnd == SCP1` | T2 | [n] |
| S15 | Evaporation walk running past the last compartment | the `compi <= NrCompartments` guard | T3 | [n] |
...
| S23 | Ponded water exhausted mid-day | transition from surface to soil evaporation | T2 | [n] |
...
| S28 | Evaporation with a restrictive layer at 0.10 m | S × F44 interaction | T2 | [n] |
...
| T07 | Root zone shallower than compartment 1 | partial-compartment uptake | T2 | [n] |
| T08 | Root zone exactly on a compartment boundary | boundary of the weighting walk | T2 | [n] |
| T09 | Root zone deeper than the compartments | uptake beyond the profile | T2 | [n] |
...
| T12 | Uptake limited by `SinkMajor` | maximum extraction with stress | T1 | [n] |
| T13 | Uptake limited by `SinkMinor` | required extraction without stress | T1 | [n] |
...
| T17 | Surface water insufficient for Tpot | `Tact < KsReduction*Part*Tpot` | T1 | [n] |
| T18 | KcTr,x 0.8 vs 1.15 vs 1.30 | crop coefficient sensitivity | T2 | [n] |
...
| U06 | `DTheta >= DThetaMax` | the per-compartment cap | T1 | [n] |
...
| U09 | CRa/CRb from the file (v ≥ 4.0) | file-supplied capillary parameters | T1 | [n] |
| U10 | CRa/CRb derived by `DetermineParametersCR` (v < 4.0) | the version-gated default path | T2 | [n] |
...
| U15 | Horizontal inflow into a drier compartment | the theta comparison branch | T1 | [n] |
| U16 | Horizontal inflow with an EC difference | `ECeComp != ECiAqua` | T1 | [ ] |
| U17 | Horizontal inflow with matching EC | the no-salt-flow branch | T2 | [n] |
...
| V12 | Salt entering from a saline water table | `SaltIn` via horizontal inflow | T1 | [n] |
...
| V18 | `Macro` (= round(FC)) governing macropore flow | macropore branch | T2 | [n] |
| V19 | `UL` and `Dx` from `SAT` and `SC` | the cell geometry | T2 | [n] |
| V20 | Salt with surface storage present | salt in ponded water | T2 | [ ] |
| V21 | Salt across a `KeepSWC` run boundary | salt carried between runs | T1 | [n] |
| V22 | Full leaching to a non-saline profile | complete salt removal | T1 | [ ] |
| V23 | Salt balance closure over a full season | `SaltIn − SaltOut` vs storage change | T1 | [n] |
| V24 | ECe → ECsw conversion in the root zone | `DetermineRootZoneSaltContent` | T1 | [ ] |
| V25 | `DetermineSaltContent` from an ECe input | `.SW0` ECe → cell salt | T1 | [n] |
...
Roughly fifty rows in the kernel groups name a branch that no combination of
input files can reach. They fall into three kinds:
...
They are marked `[n]` rather than left as `[ ]` so the outstanding count means
work that can actually be done. Reaching them needs instrumentation — a debug
build writing internal state, or unit tests calling the routines directly — which
is a different tool from this suite. `tests/assets/gen_cases_kernel2.py` carries
the full list with a one-line reason each.
