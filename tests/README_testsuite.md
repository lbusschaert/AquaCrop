...
does not change the model: nothing in `src/` is touched.
...
make -C src                            # 2. build AquaCrop
python3 tests/runner/run_tests.py -j 8 # 3. run every case
```
...
- For the results notebook: **pandas**, **numpy** and **matplotlib**, and
  optionally **ipywidgets** for the drop-down menus.
- A build of AquaCrop at `src/aquacrop` (`make -C src`).

The runs are small, but there are many. Use several cores: with 36 cores the
whole suite takes about a minute.

---

## 2. Bring the suite into your branch

You are working on your own branch (a fix, a new feature, a refactor). Merge
the suite into it:
...
2. **Ask whether it makes sense.** A fix to capillary rise should move the
   cases with a water table (groups G and U), not a rainfed crop. If cases move
   that have nothing to do with your change, look closer.

3. **If an error or a broken rule shows up**, your change has introduced a bug.
   Fix the code, not the test.

4. **If the changes are what you intended**, update the references (next
   section) and commit them together with your change.
...
overwritten, the old behaviour cannot be compared any more.
...
1. Copy the input files into the GUI's DATA folder, and `Q15.PRM` and
   `Q15.PPn` into its project folder.
2. In the GUI, switch on the output you want to compare. Output selection is a
   GUI setting and is not saved in the project. Daily outputs 3 and 5 show the
   profile depth and the water content per compartment, with the depth of each
   compartment on the line under the header.
3. Run the project and compare. The GUI prints season totals as whole numbers,
   so expect differences in the last digit only.
