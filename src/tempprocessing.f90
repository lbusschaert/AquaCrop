...
subroutine BTransferPeriod(TheDaysToCCini, TheGDDaysToCCini,&
    ...
    do Dayi = 1, L1234
        ...
        elseif (GetTemperatureFile() == '(External)') then 
            Tndayi = GetTminRun_i(GetCrop_Day1()-GetSimulation_FromDayNr()+Dayi)
            Txdayi = GetTmaxRun_i(GetCrop_Day1()-GetSimulation_FromDayNr()+Dayi)
...
real(sp) function Bnormalized(TheDaysToCCini, TheGDDaysToCCini,&
     ...
     do Dayi = 1, L1234
         ...
         elseif (GetTemperatureFile() == '(External)') then 
             ...
             Tndayi = GetTminCropReferenceRun_i(i)
             Txdayi = GetTmaxCropReferenceRun_i(i)
             GDDi = DegreesDay(Tbase, Tupper, Tndayi, Txdayi, &
                                    GetSimulParam_GDDMethod())
         else
            read(fTemp, *, iostat=rc) Tndayi, Txdayi
...
subroutine CreateTnxReference365Days()
    ...
    if ((FileExists(GetTnxReferenceFileFull())) .OR. (GetTnxReferenceFile() == '(External)')) then
        ...
        do DayNri = 1, 365
            ...
            Tlow = roundc(100*TminDataSet_temp(i)%Param, mold=int32)/100._sp
            Thigh = roundc(100*TmaxDataSet_temp(i)%Param, mold=int32)/100._sp
